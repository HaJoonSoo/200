...
Date.daysDiff = (date1, date2) => {
    ...
    const second = Math.floor((diff = diff / 1000) % 60);
    const minutes = Math.floor((diff = diff / 60) % 60);
    const hours = Math.floor((diff = diff / 60) % 24);
    const days = Math.floor(diff / 24);
    return `${days} days, ${hours} hours, ${minutes} minutes , and ${second} second`;
}
...
// date1과 date2 중 하나라도 Date 객체 인스턴스가 아닌 경우, 빈 문자열을 반환하며 함수를 종료시킨다